...
## Copy Cook Logs：PowerShell 变量后的冒号

<memory category="troubleshooting">
`PL_BuildProjectWindows` 的 `Copy Cook Logs` 使用 Windows PowerShell；双引号中的 `$dst:` 按驱动器限定变量语法解析，不是“变量值 + 字面冒号”。
冒号后没有合法变量名时会报 `InvalidVariableReferenceWithDrive`，整段脚本在执行前即解析失败，而非复制动作的 I/O 失败。
变量插值后需紧接字面冒号时，必须显式界定变量边界，如 `${dst}:`。
</memory>

## Sync 固定基线：have-list 最大 CL 不是同步进度

<memory category="core-rules">
`PLN_TaskSyncCyanCookDepot` 的同步校验入口是 `DevOps/P4SyncWorkspace.py`。
Perforce have-list 不保留已同步删除的文件；目标 CL 只有删除时，正确同步后 `#have` 的最大 CL 仍可小于目标。
因此不能用 `max(CL of ...#have) == pinned baseline` 判定同步是否到位。
固定基线校验应在同一 client、同一文件范围执行 `p4 sync -n "<workspace>/...@<baseline>"`；
仅在命令正常完成且无待同步操作时，才记录该固定基线为 `latestCL`，不能改记较小的 have CL 或忽略校验异常。
</memory>
