...
metadata:
  openclaw:
    homepage: https://github.com/JimLiu/skills#post-to-wechat
    requires:
      anyBins:
        - bun
        - npx
...
| Profile dir | Shared profile at `skills/chrome-profile` |
...
| API credentials | Follow guided setup in Step 2, or set in `.env` |
...
**API selected + missing credentials** → run guided setup per `references/api-setup.md` (writes to `.env`).
