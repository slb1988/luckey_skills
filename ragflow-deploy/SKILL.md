...
- **SSRF 拦截、JWT 失效、Worker 频繁重启、任务队列丢失（文档假 RUNNING）、MySQL binlog 暴涨等运行故障** → 读 [排障参考](references/troubleshooting.md)
- **解析任务架构与配置配对**（派发、状态聚合、行镜像、Redis/Compose 生命周期）→ 读 [解析任务与持久化机制](references/troubleshooting.md#解析任务与持久化机制)
- **排障** → Step 1 + 查日志
