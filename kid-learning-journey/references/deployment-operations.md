...
- `flask daily-math [--date YYYY-MM-DD] [--learner-id N] [--limit N]`: generate review-only dated math drafts. Install one serial periodic invocation (recommended every five minutes) in the same service environment; the code does not register a timer. Automatic daily work starts after 00:10 in `FAMILY_TIMEZONE`; Sunday after 19:00 adds one weekly digest to `MemoryOutbox`. Explicit dates support today through the next 14 days. A guardian must still publish. Verify a real receipt, next execution and idempotent retry on deployment.
- `DAILY_MATH_ENABLED=false` pauses new generation, not existing published work; `MATH_LLM_DAILY_LIMIT` defaults to 2 calls per learner/rolling 24 hours and shares the existing LLM configuration. Missing/invalid/over-quota providers fall back to rule drafts. Apply migration `d260913math01` after a consistent backup; use `uv sync --frozen` to retain the pinned FSRS dependency.
- `flask sync-memory`: retry Memory Hub outbox delivery; a generated math digest is not proof of Hub delivery.
