...
- `TodayView.vue` shows completed daily math as finished; when no plan is published, server `math_practice` identifies today's completed/resumable free session instead of opening another set. `PracticeView.vue` accepts `?session=<id>` for authorized read-only resumption and immediately shows a completed session's result. A published plan remains separate from free-practice completion. `DailyMathSettings.vue` filters versions by selected date, distinguishes refresh from regeneration, offers `auto/harder/advance` and a tomorrow shortcut, and allows guardians to regenerate/publish a new revision even when an earlier version has started or completed. It explains that the current daily entry changes on publication while prior questions and attempts remain available. In-page generation retries reuse the queued plan ID and original idempotency key. Its guardian-only question-count form loads the learner setting from the server (default 20; integer 3–50), prevents edits while busy, restores the last confirmed display on failure and offers a fresh read. Saving applies to newly generated drafts/new free sessions, not existing exercises; the form remains usable after today's practice starts or completes; applying changed questions to today's entry requires a newly generated revision and explicit guardian publication.
