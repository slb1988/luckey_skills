...
- LLM output only chooses catalog skills/approved resources and a rationale. Invalid/unavailable/over-quota output falls back to a rule draft. A two-minute lease and bounded retries protect generation; a committed `AIJob` reserves quota and records the exact input before the external request. No provider call holds a SQLite write transaction. Only a guardian can publish. Changes after generation require a new revision, not silent replacement. Publication supersedes the prior active plan without rewriting its questions, session, attempts or completion; the today entry uses the new published revision. Explicitly opening a superseded plan can resume its existing session but cannot create a new one.
