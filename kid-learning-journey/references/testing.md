...
`test_math_settings.py` and `test_math_settings_migration.py` cover default/custom/odd question counts, bounds and guardian access, idempotent setting updates, unchanged existing plans and attempts, native upgrade from `d260913math01`, and settings surviving a file-backed database restart. `MathQuestionCount.spec.ts` covers loading, saving, validation and response-loss recovery; the daily-math E2E verifies a 20-question completion followed by saving 21, regenerating/publishing the same day, starting the new session and still reopening the old completion on another device, then generating tomorrow. `test_math_republish.py` covers publishing a pending draft after the previous version starts/completes, guardian-only started-date regeneration, unchanged old evidence, idempotent publication and refusal to start an unused superseded version.
