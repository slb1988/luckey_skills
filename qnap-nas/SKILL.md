...
- **CPU 占用地图（2026-09-07 实测）**: <memory category="core-rules">4 核 J4125 常态 load 11~15 而瞬时 CPU 仅 ~36% 忙——高 load + 低瞬时 CPU 是 1900+ 线程低水平轮转 + 内存压力的典型签名，不等于有单点失控进程，排查先看开机以来平均 CPU 而非瞬时值。慢性大头：`hal_daemon` 开机均 ~8%（QNAP 硬件抽象守护，硬盘/传感器轮询空转），缓解：`/etc/init.d/hal_daemon.sh restart`（init 会拉起）；Qsirch `fs_listener` 累计 CPU 全机第一（≈8.7h）且拖着 OpenSearch java，App Center 停 Qsirch 可同时省 CPU 和内存；dockerd + container-station 合计常态 5~10% 属正常开销。</memory>
