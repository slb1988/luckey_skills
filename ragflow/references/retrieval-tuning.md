...
同一问题多次提问得到不同答案甚至互相矛盾时，按两层分诊。多次回答**引用的文档/chunk 不同** = 每次送入的 `{knowledge}` 不同，问题先从检索层开始。

**检索层随机源：**

- `prompt_config.keyword` 开启时，查询扩展由 **LLM** 完成——LLM 非确定，每次扩展出的关键词不同，term 匹配命中的 chunk 集合随之漂移。这是检索层最大的随机源，排查抖动时首先关掉它。
- 查询中的词在知识库**不存在**时（如用户问“大中小”而库里无此词），0.7 的 keyword 权重信号完全落空，检索退化为在一堆近义 chunk 中抖动，`similarity_threshold` 边缘的 chunk 时进时出。
- 表格类文档用 `naive` chunking 时，目标行被滑动块随机合并/拆开，进一步加剧命中漂移。

**生成层随机源：**

- temperature > 0 时措辞漂移只是表象；更危险的是 system prompt 缺少“仅依据知识库、没有就明说、禁止推测”的硬约束时，LLM 会**自行归纳知识库中不存在的映射关系**（幻觉），且每次编得不同。

**隔离定位法（先分诊再修）：**

绕开 LLM，直接调 retrieval API 用同一问题连查 5 次：

- chunk 集合不一致 → 检索层问题：关 `prompt_config.keyword`、表格文档改 `table` chunking、调大 `top_n` 减少边缘截断抖动、用 `auto_keywords`/`auto_questions` 把专名锚进高倍率索引字段。
- chunk 一致但答案不一致 → 纯生成层问题：temperature 设 0，system prompt 加禁止推测硬规则（只放通用规则，项目知识走 Memory）。
