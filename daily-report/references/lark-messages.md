...
走 lark-cli 路径时，除 DailySucc 合并条目外，**必须在日记中追加「## 飞书消息摘要」一节**：按群逐条归纳当日讨论内容（群名/消息数/主要内容，含用户未发言的群），放在 DailySucc 之后、App 使用时长之前。只写与用户相关的条目不等于总结了飞书消息（2026-09-18 用户指出缺失后补加）。
