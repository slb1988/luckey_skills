...
- 飞书日报提到的 CL 不一定是当日提交（可能是前一晚的提交）。若飞书引用的 CL 不在当日 P4 查询结果中，跨日 `p4 describe -s <CL>` 核对实际提交时间，日报中如实标注（如「昨晚入库」），不要冒充当日提交。
- 反向同样成立：当日查询结果里的 CL 也可能不属于当日工作——`describe` 显示提交时间在凌晨（如 00:0x）的 CL 属前一晚收尾，标注「凌晨入库」，不计入当日白天产出。日期窗口匹配 ≠ 当日工作。
- 飞书引用的 CL 号可能根本不存在（pending CL 提交时重编号，或摘要记错）。`describe` 报 no such changelist 时，按描述关键词在当日 changes 列表里反查真实 CL（实例：飞书记 wp-actor-config 为 CL 126551，实际入库 126560），日报中标注实际 CL 号，不要照抄不存在的号。
- `changes -u sunlaibing` 会带回 AI Review 流水线工作区（autoserver-MainDev-review）的提交——提交账号是本人但 workspace 不同。按 workspace 名归因到「AI 代码 Review」条目下，不要当作手动提交另立条目。
- 同理会带回 AUTOMERGE_MainDev 工作区的提交；该工作区不一定都是自动合并——本人可能从该工作区手动提交（实例：CL 124492 记忆系统改动经 AUTOMERGE 工作区入库）。按 CL 描述内容归并入对应条目，不要仅按 workspace 名另立「自动合并」条目。
- 飞书记的 CL 号本身可能记错或根本不存在（`describe` 报 no such changelist，飞书总结是 AI 生成的，会错记/幻觉 CL 号）。此时按飞书描述的工作内容在当日 `changes` 结果中反查实际 CL，日报中如实标注更正（如「飞书记 CL 126551，实际入库 CL 126560」），不要照抄错误 CL 号。
- 飞书提到的协作/工作事项若当日三路 P4 均查无对应 CL（远程协助他人、纯讨论类工作常见），不要强行关联相似描述的 CL，按飞书语义如实记录、不加 CL 子项即可。
- CICD 服务器（192.168.2.13）的查询结果可能含 TeamCity 自动化代提交的 CL（如 BlueprintCheck 触发器变更，实例 CL 981），并非本人当日工作——识别后整体排除，不作为条目也不作子项写入日报。
- P4 有、飞书未提及的多条同主题 CL（流水线批量迭代常见），归并为一条「XX 其他迭代」条目、CL 作缩进子项，不要逐条独立列出，也不要因飞书未提及而丢弃。
</memory>
...
**判断规则**（以电脑开机活跃时间为主参考，工作时段一般为上午到傍晚）：
- **上班时间**：当天第一个白天 not-afk 段的开始时间（00:00–05:00 的段属前一晚跨午夜活动，不算）
- **下班时间**：从上午延续到傍晚的主工作段结束时间（通常落在 18:00–20:00）。午休/晚饭等中断段（< 2h）不切断工作日；不要机械取"最长连续段"——它可能在下午中段就结束，但实际工作延续到了晚间
- 下班后 20:00 之后的零散活跃段视为下班后活动，不计入；跨午夜的段单独注明

<memory category="common-patterns">
- 主工作段本身可能延续跨午夜：若 not-afk 活跃从上午起无 ≥60min 间断一直延续过 00:00，下班时间取该段实际结束时间并标注「跨午夜」，不要套用 18:00–20:00 窗口截断，也不要当作「下班后零散活跃」丢弃。可用当日最后一个 P4 CL 的提交时间跨源佐证（如最后 CL 23:54 入库 ↔ 活跃段 00:03 结束）。
- 与主工作段完全分离的晚间活跃段（晚饭后重新开始的数小时连续活跃，如 19:47–23:46）若有晚间 P4 CL 跨源佐证属实际工作，在「工作时间」中单独注明（如「晚间工作：HH:MM–HH:MM」），不要当作「下班后零散活跃」直接丢弃；无佐证的零散段才不计入。
- 主工作段之前的孤立短暂活跃段（如早间 <30min 的段，与主段间隔 ≥60min）不算上班时间——上班取上午起延续的主工作段开始时间，不要机械套用「第一个白天 not-afk 段」（实例：08:30 有 18m 短暂活跃，实际上班 10:54）。
- 反向同样成立：与主工作段分离但较长（≥1h）的早间活跃段，若有该时段 P4 CL 跨源佐证属实际工作（实例：06:21–08:59 段，06:23 起多条 CL 入库），上班取该段开始时间，不要因与主段分离而丢弃。
</memory>
