...
它不是简单的原始 shelf diff，因为评审必须看到与当前基线合并后的实际结果。`p4 diff2` 只比较 depot revision，不读取 workspace merged 内容；`p4 fstat` 用于核映射，`p4 diff -du` 仅在 have 等于本轮固定 baseline 时适用，显式 baseline 仍用 `p4 print`。
...
1. 先下载近期真实 build 的 provenance，确认是否命中；若当前代码已处理同路径，不重复修复；
...
旧 Runner 会删除固定 session 及 sidecar 后重建；该描述不能覆盖可能新增的 Session 实现。执行前核当前 Runner/Session 入口、Review 绑定和实际 session 证据，不能从 `pi_session.jsonl` 的存在或文件名推断已有／没有长期续接。
