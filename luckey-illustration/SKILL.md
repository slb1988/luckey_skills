---
name: luckey-illustration
description: 生成 Luckey 风格的中文正文配图。用于用户要求为中文文章、帖子、博客、Notion 文档、工作流文档、方法论、流程、结构、状态、隐喻或观点生成“Luckey”“小Key”“钥匙豆”“手绘”“正文配图”“文章插图”“配图建议”“shot list”“去标题/改图”等任务；默认使用 Luckey 小Key IP：白色豆形小人、黑色墨镜、头顶金色钥匙、纯白手绘、少量红橙蓝批注、简洁清爽但天马行空的视觉风格。
---
...
默认视觉 IP 是“Luckey 小Key”：白色豆形小人、黑色墨镜、头顶一把小号金色钥匙、短手短腿、冷静表情，认真做一件荒诞但成立的事。小Key必须参与画面的核心动作，不能只是站在旁边当装饰。
...
- `references/xiaokey-ip.md`：Luckey 小Key IP 的形象、性格、动作库和禁忌。
...
- `assets/examples/`：只作低频视觉校准，不进入默认生成路径。优先参考 01、02 两张新版 Luckey 示例的角色比例、墨镜、头顶钥匙、线条密度和留白。不要照抄这些案例的构图、物件或标注。
...
不要复刻过往案例。案例只提供风格密度和小Key参与方式，不能直接复用“传送带断点 / 小Key拉线 / 素材鱼 / 盖章工具箱 / 常见坑路径”等已有构图，除非用户明确要求复刻某张图。每次都要从当前文章重新发明一个奇怪但成立的隐喻。
